<template >
    <tr v-for="(car, id) in cars" :key="id">
        <td>{{ id+1 }}</td>
        <td>{{ car.company }}</td>
        <td>{{ car.model }}</td>
        <td>{{ car.price }}</td>
        <td>
            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" :fill="car.color" viewBox="0 0 512 512" id="car">
                <path
...
<script>
import deleteCar from '@/components/cars/Delete.vue'
export default {
    props: {
        cars: {
            type: Array,
