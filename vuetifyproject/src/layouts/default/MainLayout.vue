<template>
    <v-card class="mx-auto" color="grey-lighten-3">
        <v-layout>
            <span :class="this.$currentTheme() + '-bg-img'"></span>
            <v-app-bar height="70" color="transparent">
                <template v-slot:image>
                    <v-img :class="this.$currentTheme() + '-gradient'"></v-img>
                </template>
                <v-app-bar-title style="font-size: 27px;" :class="this.$currentTheme() + '-text-color'" class="mx-5">
                    <v-icon  :class="this.$currentTheme()+'-text-color'">
                        <svg xmlns="http://www.w3.org/2000/svg" width="27" height="27" :class="this.$currentTheme() + '-text-color'"
                             viewBox="0 -60 512 512" id="car">
                            <path
                ...
                <v-spacer style="width: auto;" :class="this.$currentTheme() + '-text-color'" class="d-flex justify-center">
                    <v-btn @click="() => { this.$router.push('/') }" class="me-6" style="font-size: 20px; ">
                        {{ $t('navbar.cars') }}
                    </v-btn>
                    <v-btn @click="() => { this.$router.push('/settings') }" class="me-6" style="font-size: 20px;">
                        {{ $t('navbar.settings') }}
                    </v-btn>
                    <v-btn class="me-6" style="font-size: 20px;">
                        {{ $t('navbar.about') }}
                    </v-btn>
                    <v-btn @click="() => { this.$router.push('/news/Sport') }" class="me-6" style="font-size: 20px;">
                        {{ $t('navbar.news') }}
                    </v-btn>
                </v-spacer>
                <span class="fz-20" :class="this.$currentTheme() + '-text-color'">
                    <v-icon>
                        mdi-account
                    </v-icon>
                    {{ userName }}</span>
                <v-btn class="text-h6 mx-8" height="40" color="error" @click="logOutUser" variant="flat">
                    {{ $t('navbar.logout') }}
                </v-btn>
                <theme></theme>
                <v-container style="width: 120px;" :class="this.$currentTheme() + '-text-color'">
                    <v-select class="mt-5 me-2" v-model="this.$i18n.locale" :items="this.$i18n.availableLocales"
                        density="compact">
                    </v-select>
                </v-container>
            </v-app-bar>
...
import { locationHandler } from '@/i18n/index'
...
export default {
    ...
    methods: {
        ...
        changeLanguage() {
            locationHandler(['navbar', 'sidebar', 'subSidebar', 'cars', 'settings'])
        }
    },
    computed: {
        userName() {
